...
Paradex maintains a Mark Implied Volatility that reflects a fair estimate of market IV even when the perpetual option may be illiquid.

```mermaid
flowchart TD
  A["Best bid/ask prices"] -->|Internal| E["Mark IV Calculator"]
  B["Last trade price"] -->|Internal| E
  C["Mid price"] -->|Internal| E
  D["External IV (Deribit)"] -->|External| E
  E --> F["Mark IV"]
  
  classDef internal fill:#d4f1f9,stroke:#05a4c4
  classDef external fill:#ffdab9,stroke:#ff8c00
  classDef output fill:#d8f3dc,stroke:#2d6a4f
