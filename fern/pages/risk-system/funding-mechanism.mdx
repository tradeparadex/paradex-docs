...
|-----------|---------|---------|
| `baseline_rate` (interest_rate) | 0.01% per 8h | Rate the market converges toward when basis ≈ 0. |
| `clamp_rate` | 0.05% per 8h | Maximum pull of the Baseline toward or away from Fair Basis. |
| `max_funding_rate` | 5% per 8h | Cap on the raw rate. |
...
| `max_tob_spread` | 1% | Relative spread above which a tick is considered illiquid. |
