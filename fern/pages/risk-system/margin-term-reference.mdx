...
= \text{USDC Balance} * \text{USDC Oracle Price}
$$
...
& \sum\limits_{j=1}^{m} \bigg(
  \small\text{Position Notional}(D_j) \\
& \quad - \small\text{Position Cost}(D_j) * \small\text{USDC Oracle Price} \\
& \quad + \small\text{Accrued Funding}(D_j)
\bigg)
\end{align*}
$$
...
_Note :_ Calculation of IMR and MMR in a cross-margin account can be found in the [Cross Margin Requirement](cross-margin-requirement) page. For portfolio margin accounts, see the [Portfolio Margin](portfolio-margin) page.
...
$$
\begin{align*}
\text{Free Collateral Balance} = \min\big(
& \text{Account Value} - \text{Account IMR}, \\
& \text{Account USDC Balance}\big)
\end{align*}
$$
