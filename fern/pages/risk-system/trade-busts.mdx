...
After a bust, the account is automatically placed into **RESYNC** mode, which synchronizes the off-chain state with the on-chain state to stop new busts from being generated. Trades already inflight to the chain will still bust since they were submitted before the resync began.
