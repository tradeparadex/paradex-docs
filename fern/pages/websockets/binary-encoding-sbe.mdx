...
As of 21 September 2026, Paradex requires SBE for public channel payloads. JSON is no longer served for public channels. Connections must include the schema parameters described below, or use the Python SDK with `sbe_enabled=True`.
</Warning>

Public channel payloads are delivered as binary [Simple Binary Encoding](https://github.com/real-logic/simple-binary-encoding) frames instead of JSON. SBE is a FIX Trading Community standard designed for low latency market data, and typically reduces message size substantially compared with the equivalent JSON.

SBE frames carry the same field content as the JSON payloads they replace. The JSON-RPC control plane and private channel payloads remain JSON (see [What stays JSON](#what-stays-json)).
...
Decoders are generated from the Paradex SBE schema, [`paradex_1_0.xml`](https://github.com/tradeparadex/paradex-py/blob/main/paradex_py/api/sbe/paradex_1_0.xml), using the [Real Logic SBE tool](https://github.com/real-logic/simple-binary-encoding), the reference implementation of the standard, which supports Java, C++, C#, Go and Rust:
...
The [paradex-py](https://github.com/tradeparadex/paradex-py) SDK ships a maintained decoder and negotiates the schema for you. SBE support for schema 1:1 is available from v0.7.0:
...
ws_client = ParadexWebsocketClient(env=PROD, sbe_enabled=True)
```
