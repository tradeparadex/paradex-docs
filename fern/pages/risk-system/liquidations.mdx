...
Paradex employs **Partial Liquidation** of unhealthy accounts and attempts to minimize impact on the user's assets and positions while minimizing the platform risk.
...
| ----------------- | ------- | ------------------- | ----------------- | ---------------- |
| 20%               | 71.04   | 12.43               | 67.57             | 105.1%           |
| 40%               | 53.28   | 24.86               | 55.14             | 96.6%            |
| 60%               | 35.52   | 37.3                | 42.7              | 83.2%            |
| 80%               | 17.76   | 49.73               | 30.27             | 58.7%            |

In this example, the insurance fund will liquidate 60% of the user's positions to bring the account's Margin Ratio below 90% and will receive 37.3 USDC as a liquidation penalty (corresponding to a 70% liquidation fee).

Alice realized a loss of 60% \* 920 = 552 USD and is left with 1,000 - 552 - 37.3 = 410.7 USDC collateral

Alice is also left with a long 0.04 BTC-USD-PERP and a short 0.4 ETH-USD-PERP positions which corresponds to an unrealized PnL of -368 USDC.

The new account value of Alice is 410.7 - 368 = 42.7 USDC, the new margin ratio is now 83.2%. The insurance fund only took 37.3/88.8 = 42% of the maintenance margin requirement as a penalty to perform this liquidation and make the account healthy again.
...
The Insurance Fund serves as a backstop to minimize the risk of socialized losses. It prevents the propagation of risk from bankrupt accounts toward the rest of the market.

## Tutorial: Checking Insurance Fund Balance on Paraclear Contract

<Info>
The insurance fund on the Paraclear contract holds funds to cover losses in case of liquidation. It doesn't show its balance on the block explorer because the funds are not held in the USDC contract itself.
</Info>

Here's how you can check the balance **on-chain** :

1. **Go to Paraclear Contract:**\
   Open the following link to access the Paraclear contract on the block explorer: [Paraclear Contract on Block Explorer](https://app.paradex.trade/explorer/contract/0x03ca9388f8d4e04adecbd7b06b9b24a33030a593522248a7bddd87afc0b61a0c)
...
4.  **Input the Required Addresses:**\
    ...
    `0x24d52bb68763343c4f73b63e3723007e00b9f81d2f36d0036c3590ad32062c9`
...
> **Example:**
>
> \- Insurance fund address: 0x24d52bb68763343c4f73b63e3723007e00b9f81d2f36d0036c3590ad32062c9
>
...
When Paradex Insurance Fund is bankrupt due to large amounts of unprofitable liquidations, [Socialized Losses](socialized-losses) will occur as a last level of backstop.
