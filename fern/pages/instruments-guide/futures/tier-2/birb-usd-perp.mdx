---
---
export const contractName = "BIRB-USD-PERP";
export const productType = "BIRB Perpetual Future";
export const settlementCurrency = "USDC";
export const baseCurrency = "BIRB";
export const quoteCurrency = "USD";
export const fundingPeriod = "8.0 hours";
export const tickSize = "0.00001 USD";
export const orderSizeIncrement = "1 BIRB";
export const minOrderValue = "10 USD";
export const maxOrderSize = "1,700,000 BIRB";
export const maxOpenOrders = "75";
export const positionLimit = "5,600,000 BIRB";
export const priceBandFactor = "20.0%";
export const maxFundingRate = "10%";
export const ewmaFactor = "20%";
export const imf = "20%";
export const mmf = "50%";

<Markdown src="../../../../snippets/contractTemplate.mdx"/>
