...
subtitle: Some AI clients — like Claude.ai (web) and ChatGPT Desktop — cannot run local programs and need a public URL to connect to the MCP server.
---
...
4. Confirm the `[[services]]` block in `fly.toml` listens on port `8080` with health check path `/health`:
