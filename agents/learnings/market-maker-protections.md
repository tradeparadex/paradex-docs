...
- The initial product spec differed from the implementation. Verified behavior (keep these when editing):
  ...
  - The arriving order's own flag still gates entry: `MMP_FROZEN` and `MMP_NOT_CONFIGURED` still apply to incoming MMP orders. The stale Greeks check at arrival only applies to orders that can rest (an MMP `LIMIT` `IOC` is not canceled for stale Greeks); not documented on the page.
