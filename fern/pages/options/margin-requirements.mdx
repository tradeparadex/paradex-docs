...
Paradex will soon enable Portfolio Margin which will unlock much higher capital efficiency for hedged accounts. Portfolios with low risk will benefit from much lower margin requirements under Portfolio Margin Mode
...
| **Long ITM Fraction**                                | `long_itm_imf` **= 10%**            | `long_itm_mmf` **= 5%**            |
| **Short ITM Fraction**                               | `short_itm_imf` **= 7.5%**          | `short_itm_mmf` **= 3.75%**        |
| **Short OTM Fraction**                               | `short_otm_imf` **= 5%**            | `short_otm_mmf` **= 2.5%**         |
...
If Option Mark Price = 30 → IMR = 10% x Spot Price = 10 and MMR = 5% x Spot Price = 5
...
- OTM Call Option with Strike Price = 106
  - OTM Amount = 5
  - IMR = max(7.5-6, 5) = 5
  - MMR = max(3.75-6, 2.5) = 2.5
- ITM Put Option with Strike Price = 106
  - OTM Amount = 0
  - IMR = max(7.5, 5) = 7.5
  - MMR = max(3.75, 2.5) = 3.75
...
- OTM Put Option with Strike = 40 and Option Mark Price = 25
  - IMR = min(short_otm_imf * Spot Price, short_put_cap_imf * Strike Price) = min(5, 20) = 5
  - MMR = min(short_otm_mmf * Spot Price, short_put_cap_mmf * Strike Price) = min(2.5, 20) = 2.5
