...
| More than 60 resets per minute per account, or a short burst of resets over the burst limit | Rejected with HTTP 429. |
...
| MMP flag set on an order type other than `LIMIT`, an unsupported market or a block trade leg | Rejected with `INVALID_PARAMETER` and an explanatory message. |
...
| `MMP_DISABLED` | Resting MMP orders are canceled because MMP was disabled for the underlying, or because MMP is temporarily unavailable, for example during a restart. |
...
| Field | Meaning |
| --- | --- |
| `trip_reason` | Trigger reason: `SIZE`, `DELTA` or `VEGA` when that limit was exceeded, or `INVALID_FILL` when a fill could not be counted and the underlying was frozen as a precaution. |
...
[`paradex-py`](https://github.com/tradeparadex/paradex-py) 0.7.1 is the first version that supports the `MMP` order flag. Earlier versions cannot parse MMP flagged orders or protected block trade legs, including your own. Upgrade to 0.7.1 or later before placing MMP orders or opting in to block trade protection.
