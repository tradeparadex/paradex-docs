...
## Identifying FastFills Fills

Fills that qualify for the FastFills discount are marked with a `fastfill` flag in the fill response. When querying fills via the [List Fills](/api/prod/account/list-fills) endpoint, check the `flags` array for the `fastfill` value.
...
These fields allow you to see how much retail size is available on top of the book, helping you understand potential FastFills opportunities. Query the [Get Orderbook](/api/prod/markets/get-orderbook) endpoint to access this information.
...
- API traders matching with retail passive orders benefit from the FastFills discount while respecting this priority system
