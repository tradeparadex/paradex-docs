...
In Cross Margin, the account margin requirement is the total sum of margin requirements across markets. For portfolio-level risk evaluation with reduced margin requirements, see [Portfolio Margin](portfolio-margin).
...
i.e.
$$
\text{Open Notional} = \sum\limits_{\text{market}~m} \text{Open Size}(m) * \text{Mark Price}(m)
$$
