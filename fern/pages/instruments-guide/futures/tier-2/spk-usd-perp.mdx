---
---
export const contractName = "SPK-USD-PERP";
export const productType = ̰"Linear Perpetual Future";
export const settlementCurrency = "USDC";
export const baseCurrency = "SPK";
export const quoteCurrency = "USD";
export const fundingPeriod = "8 hours";
export const tickSize = "0.00001 USD";
export const orderSizeIncrement = "1 SPK";
export const minOrderValue = "50 USD";
export const maxOrderSize = "2,000,000 SPK";
export const maxOpenOrders = "75";
export const positionLimit = "7,600,000 SPK";
export const priceBandFactor = "20%";
export const fundingClampingRate = "5%";
export const ewmaFactor = "15%";
export const imf = "20%";
export const mmf = "50%";

<Markdown src="../../../../snippets/contractTemplate.mdx"/>
