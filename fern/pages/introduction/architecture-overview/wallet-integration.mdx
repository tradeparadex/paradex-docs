...
Paradex, a [Starknet Appchain](/docs/getting-started/architecture-overview/starknet-first-appchain), incorporates separate public/private keys from your connected wallet (ie: Ethereum L1). To interact with Paradex, here's a simplified overview:
...
4. **EVM Wallets**: EVM wallets (e.g., MetaMask) can onboard by signing a [SIWE (Sign-In with Ethereum)](https://eips.ethereum.org/EIPS/eip-4361) message instead of a Starknet typed data message. The Paradex account address is deterministically computed from the ETH address. EVM accounts use Argent v0.5.0 (Cairo 1) deployed directly — no proxy wrapper is needed. After onboarding, authentication also uses SIWE messages via the `/v2/auth` endpoint.
...
<img src="../../../assets/architecture/wallet_metamask_integration.png" alt=""/>
</Frame>
...
<img src="../../../assets/architecture/wallet_key_derivation_diagram.png" alt=""/>
...
### EVM Account Onboarding

For EVM wallets, the onboarding flow differs from the Starknet-based flow described above:

- **No key derivation**: Instead of using a KDF to derive a Stark-compatible key from an Ethereum signature, the wallet's secp256k1 public key is used directly. This eliminates the deterministic signature and KDF steps entirely.
- **SIWE message**: The user signs a [SIWE (ERC-4361)](https://eips.ethereum.org/EIPS/eip-4361) message with their EVM wallet. This replaces the Starknet typed data signature used in the traditional flow.
- **Address computation**: The Paradex Starknet account address is deterministically derived using `pedersen(PREFIX, deployer=0, salt=ethAddress, classHash, hash(calldata))`, where the class hash is the EVM account class hash from system config.
- **Contract type**: EVM accounts use Argent v0.5.0 (Cairo 1) with native upgradeability, deployed directly on Starknet. This differs from Starknet-onboarded accounts which use a Cairo 0 proxy pattern.

For full API-level details on EVM authentication endpoints, see [API Authentication](/api/general-information/api-authentication).

<Warning>
The wallet you connect during onboarding is permanently linked to your Paradex L2 account and cannot be updated later. This applies to both Starknet-onboarded and EVM-onboarded accounts.

If you want to operate with a different wallet, you will need to create a new Paradex account.
</Warning>
