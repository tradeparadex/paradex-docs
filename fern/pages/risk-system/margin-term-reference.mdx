...
The USD value of the collateral that can be immediately withdrawn subject to the upper bound of the user's Account USDC Balance.

$$
\text{Free Collateral Balance} = \text{min}(\text{Account Value} - \text{Account Initial Margin Requirement}, \text{Account USDC Balance})
$$

A user's Account USDC Balance is equal to thier net USDC deposits (deposits - withdrawals) plus realized PnL (including funding), minus paid fees.
