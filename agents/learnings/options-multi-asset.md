...
- US500 options are priced and settled against the Paradex US500 Spot Oracle Price (same index as US500-USD-PERP). Equity indices (US500, US100) use only Pyth, Hyperliquid and Lighter as external sources (scores 1/1/0.7, all 24/7), plus Closing Price and Internal Price (0.5 each) during closing hours. This is documented on `us500-usd.mdx` ("Index and settlement price") and in the "Equity Indices Pricing" section of `fern/pages/trading/real-world-assets.mdx` (slug `/trading/tradfi-markets`).
- The live URL for the US500 perp page is `/trading/instruments-guide/futures/trad-fi/us500-usd-perp` (Fern slugifies the `TradFi` section as `trad-fi`).
