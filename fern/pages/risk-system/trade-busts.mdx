...
- **Margin requirement mismatch.** The off-chain and on-chain risk engines evaluate margin requirements using slightly different data snapshots. Differences in oracle prices, funding accruals, or timing between the two checks can cause a trade to pass off-chain validation but fail on-chain.
...
When a trade bust occurs due to a mismatch between off-chain and on-chain values, the account is automatically placed into **RESYNC** mode. During resync, the system synchronizes the off-chain state with the on-chain state to prevent further busts caused by the same data discrepancy.
