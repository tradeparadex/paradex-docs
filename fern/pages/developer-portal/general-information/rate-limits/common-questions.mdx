...
* Distributed Leaky Bucket Algorithm rate limiting
* This technique uses a "token bucket" system based ona maximum number of available request tokens for a given timeframe. Each request consumes one token, and tokens are continuously refilled at a fixed rate. When all tokens are consumed, further requests are blocked until new tokens become available.
...
Example: If you have a 600 requests/minute limit:
